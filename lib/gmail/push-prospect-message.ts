...
export interface PushProspectMessageArgs {
  draftId: string;
...
/**
 * Same shape as pushClientMessageToGmail, one step simpler: a prospect's email lives on
 * the prospect row itself, no client_contact join needed since a prospect isn't a client.
 */
export async function pushProspectMessageToGmail(
  db: SupabaseClient, args: PushProspectMessageArgs, gmail?: GmailClient,
): Promise<PushResult> {
  const client = gmail ?? createGmailClient(requireToken(args.accessToken));
  const now = args.now ?? new Date();

  const { data, error } = await db.from("prospect_message_draft")
    .select("id,org_id,prospect_id,subject,body,provider_draft_id")
    .eq("id", args.draftId).maybeSingle();
  if (error) throw error;
  if (!data) throw new Error("prospect message draft not found");
  const draft = data as unknown as DraftRow;
  if (draft.org_id !== args.orgId) {
    throw new Error("prospect message draft does not belong to the caller's org");
  }
  ...
  const { data: claimed, error: claimError } = await db.from("prospect_message_draft")
  ...
  const releaseClaim = async () => {
    const { error: releaseError } = await db.from("prospect_message_draft")
      .update({ provider_draft_id: null }).eq("id", draft.id);
    if (releaseError) throw releaseError;
  };

  const { data: prospect } = await db.from("prospect")
    .select("email,org_id").eq("id", draft.prospect_id).maybeSingle();
  const recipient = (prospect?.org_id === args.orgId
    ? (prospect?.email as string | null | undefined) : null) ?? null;
  ...
  const { error: updateError } = await db.from("prospect_message_draft").update({
  ...
  await logAudit({
    ...
    target: `prospect_message_draft:${draft.id}:gmail_push`,
